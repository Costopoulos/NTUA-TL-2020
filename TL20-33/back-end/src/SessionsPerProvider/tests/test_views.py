...
class TestViews(TestCase):
    ...
    def setUp(self):
        self.cli = Client()
        self.SPPr_providerID_url = reverse('SPPr_providerID') + "/" + 3
        self.SPPr_providerID_Start_url = reverse('SPPr_providerID_Start') + "/" + str(3) + "/" + 20210106
        self.SPPr_providerID_Start_Finish_url = reverse('SPPr_providerID_Start_Finish')  + "/" + str(3) + "/" + 20210101 + "/" + 20210201
    ...
    def test_SPPr_providerID_url(self):
        response = self.cli.get(self.SPPr_providerID_url)
        self.assertEquals(response.status_code, 402)

    def test_SPPr_providerID_Start_url(self):
        response = self.cli.get(self.SPPr_providerID_Start_url)
        self.assertEquals(response.status_code, 200)

    def test_SPS_stationID_Start_Finish_url(self):
        response = self.cli.get(self.SPPr_providerID_Start_Finish_url)
        self.assertEquals(response.status_code, 200)

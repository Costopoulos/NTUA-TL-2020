...
class SPEV_NoArgs(APIView):
    
    def get(self, request):
        result = Car.objects.all().count()
    ...
    def get(self, request, vehicle_id):
        if os.path.exists("softeng20bAPI.token"):
            if os.stat("softeng20bAPI.token").st_size != 0:
                rawObj = Charging.objects.raw("SELECT * FROM charging WHERE Car_id = '" + str(vehicle_id) + "'")
    ...
    def get(self, request, vehicle_id, start):
        if os.path.exists("softeng20bAPI.token"):
            if os.stat("softeng20bAPI.token").st_size != 0:
                ...
                # Run query
                rawObj = Charging.objects.raw("SELECT * FROM charging WHERE Car_id = '" + str(vehicle_id) + "' AND Start >= '" + startString + "'")
                jsonObj = getJsonObject(rawObj, Charging) # make RawQuserySet object into json
                if len(jsonObj) == 0:
                    return Response({"No data": "There is no data for the filter you have provided"}, status=status.HTTP_402_PAYMENT_REQUIRED)
                return Response(jsonObj)
    ...
    def get(self, request, vehicle_id, start, finish):
        if os.path.exists("softeng20bAPI.token"):
            if os.stat("softeng20bAPI.token").st_size != 0:
                ...
                # Run query
                rawObj = Charging.objects.raw("SELECT * FROM charging WHERE Car_id = '" + str(vehicle_id) + "' AND Start >= '" + startString + "' AND Finish <= '" + finishString + "'")
                jsonObj = getJsonObject(rawObj, Charging) # make RawQuserySet object into json
                if len(jsonObj) == 0:
                    return Response({"No data": "There is no data for the filter you have provided"}, status=status.HTTP_402_PAYMENT_REQUIRED)
                return Response(jsonObj)

...
export class ChargingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(params =>{
      this.stationID = params.get('Station_id');
    });
    this.dataService.sendGetRequest("SessionsPerStation/" + this.stationID).subscribe((data: any) => {
      console.log(data);
      this.chargings = data;
    })
  }
  ...
  sendRequest():void {
    ...
    this.chargings = null;
    this.dataService.sendGetRequest("SessionsPerStation/" + this.stationID + this.url).subscribe(
      (response) => {
...
//this.url = (this.startingDate != undefined) ? '/' + this.startingDate:'' + (this.endingDate != undefined) ? '/' + this.endingDate : ''